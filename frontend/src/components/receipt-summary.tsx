...
export function ReceiptSummary({
  ...
  return (
    <section
      ...
      aria-labelledby="receipt-data-title"
    >
      ...
      <div className="space-y-7 p-5 sm:p-6">
        ...
        <section aria-labelledby="merchant-details-title">
          <h3 id="merchant-details-title" className="font-semibold">
            Merchant details
          </h3>
          <dl className="mt-3 grid gap-3 sm:grid-cols-2">
            <Datum label="Legal entity">{data.legal_entity}</Datum>
            <Datum label="Registration number">
        ...
        <section aria-labelledby="line-items-title">
          <div className="flex items-end justify-between gap-3">
            <h3 id="line-items-title" className="font-semibold">
              Line items
            </h3>
            <p className="muted">
              {data.line_items.length} item
              {data.line_items.length === 1 ? "" : "s"}
            </p>
          </div>
        ...
        >
          <div>
            <h3 id="totals-title" className="font-semibold">
              Payment details
            </h3>
            <dl className="mt-3 grid gap-3 sm:grid-cols-2">
              <Datum label="Cash tendered">
                {amount(data.cash_tendered, currency)}
              </Datum>
              <Datum label="Change">
                {amount(data.change_amount, currency)}
              </Datum>
            </dl>
          </div>
            ...
            {[
              ...
              >
                ...
                <dd className="font-medium tabular-nums">
                  {label === "Receipt discount" && value != null ? "−" : ""}
                  {amount(value as number | null, currency)}
                </dd>
              </div>
            ))}
