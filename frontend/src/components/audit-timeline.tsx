...
function changes(
  ...
  for (const key of Object.keys(labels) as (keyof Extraction)[]) {
    ...
    if (JSON.stringify(oldValue) !== JSON.stringify(newValue)) {
      result.push({
        field: labels[key],
        before: shown(oldValue),
        after: shown(newValue),
      });
    }
  }
...
function ChangeList({ items }: { items: Change[] }) {
  if (!items.length)
    return <p className="muted mt-3">No receipt fields were changed.</p>;
  return (
    <div className="mt-4 overflow-hidden rounded-lg border">
      ...
      {items.map((item) => (
        <div
          ...
          className="grid grid-cols-[minmax(7rem,0.8fr)_1fr_1fr] gap-3 border-t px-3 py-2.5 text-sm"
        >
          <span className="font-medium">{item.field}</span>
          <span className="break-words text-muted-foreground line-through decoration-red-300">
            {item.before}
          </span>
  ...
  return (
    ...
    >
      <li className="flex items-center gap-2">
        <Check className="size-4 text-primary" /> Original evidence{" "}
        {evidence ? "confirmed" : "not confirmed"}
      </li>
      <li className="flex items-center gap-2">
        <Check className="size-4 text-primary" />{" "}
        {category ? `Category: ${category}` : "No category accepted"}
      </li>
    </ul>
  );
}
...
export function AuditTimeline({
  ...
  const events = [
    ...reviews.map((event) => ({
      ...
      date: event.reviewed_at,
      event,
    })),
    ...amendments.map((event) => ({
      kind: "amendment" as const,
      date: event.amended_at,
      event,
    })),
      ...
      <div className="flex items-start gap-3">
        ...
        <div>
          ...
          <p className="muted mt-1">
            An append-only timeline of human decisions and corrections.
          </p>
        </div>
      </div>
      {!events.length ? (
        <p className="muted mt-5">
          No human review or amendment has been recorded.
        </p>
      ) : (
        <ol className="relative mt-6 space-y-5 border-l border-border pl-6">
          {events.map(({ kind, event }) => {
            const isReview = kind === "review";
            const before = isReview
              ? event.before?.extracted_data
              : event.before?.final_data;
            const after = event.final_data;
            const itemChanges = changes(before, after);
            const categoryBefore = isReview
              ? event.before?.classification?.category
              : event.before?.category;
            if (after && event.category && categoryBefore !== event.category) {
              itemChanges.unshift({
                field: "Category",
                before: shown(categoryBefore),
            ...
            return (
              <li key={event.request_id} className="relative">
                <span className="absolute -left-[2.05rem] top-1 flex size-4 items-center justify-center rounded-full bg-white ring-4 ring-white">
                  <span
                    className={`size-2.5 rounded-full ${isReview && event.decision === "REJECTED" ? "bg-red-500" : kind === "amendment" ? "bg-sky-500" : "bg-emerald-500"}`}
                  />
                </span>
                <article className="rounded-xl border p-4">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                      ...
                      <p className="muted mt-1">
                        {event.reviewer} ·{" "}
                        {new Date(
                          isReview ? event.reviewed_at : event.amended_at,
                        ).toLocaleString()}
                      </p>
                    </div>
                    {!isReview && (
                  ...
                  <p
                    className={`mt-3 text-sm ${isReview && event.decision === "REJECTED" ? "font-medium text-red-800" : ""}`}
                  >
                    {note}
                  </p>
                  <Checks
                    category={event.category}
                    evidence={event.evidence_confirmed}
                  />
                  {isReview && event.decision === "REJECTED" ? (
                    <p className="mt-4 rounded-lg bg-red-50 p-3 text-sm text-red-900">
                      No corrected receipt data was accepted. The expense
                      remains excluded while its original evidence is retained.
                    </p>
                  ) : (
                    <ChangeList items={itemChanges} />
                  )}
                  {(event.override_reason ||
                    event.validation_issues.length > 0) && (
                    <div className="mt-4 rounded-lg bg-amber-50 p-3 text-sm text-amber-950">
                      ...
                      {event.validation_issues.length > 0 && (
                        <p className="mt-1">
                          <span className="font-medium">Validation notes:</span>{" "}
                          {event.validation_issues.join("; ")}
                        </p>
                      )}
                    </div>
                  )}
                  <details className="mt-4 text-xs text-muted-foreground">
                    ...
                    {event.reprocess_request_id && (
                      <p className="mt-2 break-all font-mono">
                        Extraction draft {event.reprocess_request_id}
                      </p>
                    )}
                  </details>
                </article>
